...
class TestDeleteProject:
    ...
    def test_delete_project_success(self, mock_supabase, mock_current_user):
        ...
        # Setup mocks
        mock_supabase.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value.data = [SAMPLE_PROJECT]
        ...
        mock_supabase.table.return_value.update.return_value.eq.return_value.execute.return_value.data = [{"status": "cancelled"}]
    ...
    def test_delete_project_with_deployed_version_forbidden(self, mock_supabase, mock_current_user):
        """Test deletion of project with deployed versions should fail."""
        # Setup mocks - project has deployed version
        mock_supabase.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value.data = [SAMPLE_PROJECT]
        mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.limit.return_value.execute.return_value.data = [{"id": "version123"}]  # Deployed version exists
